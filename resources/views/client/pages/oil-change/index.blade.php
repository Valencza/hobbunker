...
@section('title', 'Jadwal Ganti Oli')
...
<main class="p-4">
    ...
    <h2 class="anchor mb-5 mt-3">Jadwal Ganti Oli</h2>
    ...
    <section class="rounded-2xl shadow-sm position-relative card-attendance-s">
        ...
        <div class="position-absolute z-2 text-white p-4 w-100 border rounded-4 shadow-sm">
            <div class="d-flex justify-content-end">
                <a href="{{route('oil-change.create')}}" class="btn btn-success fw-bolder">
            ...
            <div class="text-white fw-bolder mt-5 mb-4">
                <h3 class="fw-bolder mb-4">Oli</h3>
            </div>
            ...
            <div class="row pb-2">
                <div class="col-6">
                    <div class="card h-100 bg-body-secondary p-3 rounded-3 my-2">
                        ...
                        <h1 class="text-primary text-2xl mt-3">{{$totalOilStock}} <span
                                class="fs-4 fw-normal">{{ucwords($oilUnit)}}</span></h1>
                ...
                <div class="col-6">
                    <div class="card h-100 bg-body-secondary p-3 rounded-3 my-2">
                        ...
                        <h1 class="text-success fs-1 mt-3">{{$latestOilChange->formatted_date ?? '-'}}</h1>
                        <h4 class="text-secondary">Pergantian Terakhir</h4>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card border-1 border-dashed rounded-2xl d-flex gap-3 flex-row justify-content-between align-items-center py-3 px-3 mb-3"
        ...
        <div class="d-flex flex-column w-100">
            ...
            <p class="text-muted m-0">Jadwal Pergantian Oli Selanjutnya pada tanggal </p>
            <p class="text-primary m-0">{{$nextChangeDate}}</p>
        </div>
    </div>
    @endif
    ...
    <section class="card shadow-sm p-4 rounded-2xl">
        <div class="d-flex justify-content-between">
            <h2 class="fs-4 mb-3 ">Riwayat Pergantian Oli</h2>
        </div>
        ...
        <div class="d-flex flex-column gap-3 mb-5">
            ...
            <div
                ...
                <div class="card btn btn-light-info p-2 border-0 rounded-2xl w-25 d-flex align-items-center">
                    <i class="bi bi-recycle text-success fs-1">
                    </i>
                </div>
                <div class="d-flex flex-column w-100">
                    <span class="fw-medium fs-5 p-0">{{ucwords($oilChange->section)}}</span>
                    <h4 class="text-muted p-0 text-secondary fs-6">
                        Tanggal :
                        <span class="text-info ml-3">{{$oilChange->formatted_date}}</span>
                    </h4>
                </div>
                <div>
                    <a href="{{route('oil-change.detail', $oilChange->uuid)}}"
                        class="btn btn-secondary px-3 py-1 fs-6">Detail</a>
                </div>
            </div>
            @endforeach

            {{$oilChanges->links('vendor.pagination.bootstrap-5')}}

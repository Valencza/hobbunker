...
@section('title', 'Tambah Penurunan Barang')
...
<main id="form-main" class="p-4">
    ...
    <h2 class="anchor mb-5 mt-3">Tambah Penurunan Barang</h2>

    <section>
        <div class="card shadow-sm rounded-2xl">
            <h3 class=" border-1 border-bottom p-4">Form Penurunan Barang</h3>

            <div class="px-4 py-3 text-capitalize">
                <form action="{{route('stock-inventory-ship.drop-item.store', $uuid)}}" method="POST">
                    @csrf
                    <div class="mb-10">
                        <label for="master_ship_uuid" class="required form-label">Kapal</label>
                        <select class="form-select  border" id="master_ship_uuid"
                            name="master_ship_uuid" readonly>
                            <option value="{{$masterShip->uuid}}" selected>{{$masterShip->name}}</option>
                        </select>
                    </div>

                    <div class="mb-10">
                        <label for="date" class="required form-label">Tanggal Penurunan</label>
                        <input type="date" id="date" name="date" class="form-control  border" value="{{$date}}"
                            readonly />
                    </div>

                    <div class="mb-10">
                        <label for="title" class="required form-label">Judul</label>
                        <input type="text" id="title" name="title" class="form-control form-control border"
                    ...
                    <h3 class=" border-1 pb-10">Detail Item</h3>
                ...
                <div class="d-flex gap-3 w-100">
                    <div id="submit-btn" class="btn btn-primary text-white w-100">Kirim Penurunan</div>
                </div>
            </div>
        </div>
    </section>
...
<script>
    ...
    function checkItemCode(item) {
        if (item.code) {
            ...
            let masterShips = @json($masterShips);

            if (check) {
                Swal.fire({
                    title: 'Invantaris sudah ditambahkan',
                    icon: 'error',
            ...
            $.ajax({
                url: "{{ route('api.stock-inventory-ship.check-inventory-code')}}",
                type: 'GET',
                data: {
                    code: item.code
                },
                success: function (response) {
                    if (response.status) {
                        ...
                        let options = masterShips.map(ship => `<option value="${ship.value}">${ship.name}</option>`).join('');
                        ...
                        $('#list-item').append(`
                            <div class="item card rounded-2xl  px-4 py-3">
                                ...
                                <div>
                                    <label for="master_ship_uuid_${index}">Tujuan</label>
                                    <select class="form-select" name="master_ship_uuid[${index}]" id="master_ship_uuid_${index}">
    ...
    $(document).ready(function () {
        $('#submit-btn').click(function () {
            ...
            if (!itemsLength) {
                Swal.fire({
                    title: 'Inventaris belum dipilih',
                    icon: 'info',
                    showConfirmButton: false
                });
            ...
            Swal.fire({
                title: 'Konfirmasi Penurunan Inventaris',
                text: "Pastikan data penurunan inventaris telah sesuai",
                icon: 'warning',

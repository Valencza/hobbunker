...
<main class="p-4" style="overflow-x: hidden">
    ...
    <form action="{{route('stock-inventory-ship.inventory.store')}}" method="POST">
        @csrf
...
<script>
    let date = JSON.parse(localStorage.getItem('ship-inventory-date'));
    let items = JSON.parse(localStorage.getItem('ship-inventories'));

    items.forEach((item, index) => {
        $('#list-item').append(`
            <div class="fs-4 mb-3 border-1 border-bottom py-4 px-4">
                <h2>${item.name}(${item.qty})</h2>
                <p class="text-secondary">Bagian: ${item.position}</p>
            </div>

            <input type="hidden" value="${item.position}" name="position[${index}]" />
            <input type="hidden" value="${item.qty}" name="qty[${index}]" />
            <input type="hidden" value="${item.uuid}" name="master_item_uuid[${index}]" />
        ...
        for (let i = 1; i <= item.qty; i++) {
            ...
            $(`#list-code-${item.uuid}`).append(`
                <section class="download-areas rounded-2xl shadow-sm card" id="download-area-${index}" style="width: 300px; height: 400px; margin: auto;">
                    <div class="d-flex flex-column align-items-center">
                        <img class="mb-3 mt-5" width="80" src="{{ asset('assets/media/logos/blueLogo.svg') }}" alt="HOB Logo">
                        <img src="${base64Image}" class="w-75 px-4" alt="">
                    </div>
                    <div class="text-center mt-5">
                        <h5>${item.name}</h5>
                        <h6 class="text-secondary">${code}</h6>
                    </div>
                </section>

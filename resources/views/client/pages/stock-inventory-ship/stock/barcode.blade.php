...
<main class="p-4"style="overflow-x: hidden">
    ...
    <form action="{{route('stock-inventory-ship.stock.store')}}" method="POST">
        @csrf

        <section id="list-item" class="card shadow-sm rounded-2xl">
        </section>

        <div id="submit-btn" class="btn btn-primary mt-4  w-100">Konfirmasi Tambah Stok</div>
    </form>
</main>
@endsection
...
<script>
    let date = JSON.parse(localStorage.getItem('ship-stock-date'));
    let items = JSON.parse(localStorage.getItem('ship-stocks'));
    ...
    $('#submit-btn').click(function () {
        Swal.fire({
            title: 'Konfirmasi Penambahan Stok Barang',

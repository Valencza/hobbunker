...
@section('title', 'Laporan Pergantian BBM')
...
<main id="form-main" class="p-4">
    <ul class="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
        ...
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item text-gray-500">Pemakaian BBM</li>
        <!--end::Item-->
    </ul>

    <h2 class="anchor mb-5 mt-3">Laporan Pemakaian BBM</h2>

    <section>
        <div class="card shadow-sm rounded-2xl">
            <form action="{{route('bbm.store')}}" method="POST">
                @csrf
                <h3 class=" border-1 border-bottom p-4">Form Pemakaian BBM</h3>
                ...
                <div class="px-4 py-3 text-capitalize">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="date" id="created_at" name="created_at" class="form-control border"
                            value="{{$date}}" readonly />
                    </div>
                    ...
                    <div class="row mb-10">
                        <div class="col-6">
                            <label for="start" class="required form-label">Jam Mulai</label>
                            <input type="time" id="start" name="start" class="form-control border" />
                        </div>
                        <div class="col-6">
                            <label for="stop" class="required form-label">Jam Stop</label>
                            <input type="time" id="stop" name="stop" class="form-control border" />
                        </div>
                    </div>

                    <div class="mb-10">
                        <label for="total" class="required form-label">Total Pemakaian (liter)</label>
                        <input type="number" id="total" name="total" class="form-control border"
                            placeholder="Masukkan total pemakaian bbm" />
                    </div>
        ...
        <div class="modal-footer">
            ...
            <button type="button" id="add-btn" class="btn btn-primary" data-bs-dismiss="modal">Tambah</button>
        </div>
        </div>
        </div>
        </div>
        ...
        <div class="d-flex gap-3 w-100 px-4 mb-4">
            <a href="{{route('bbm')}}" class="btn btn-secondary rounded-3 w-100">Batal</a>
    ...
    $(document).ready(function () {
        ...
        $('#submit-btn').click(function () {
            ...
            if (!$('#master_meachine_uuid').val()) {
                Swal.fire({
                    title: 'Tipe mesin belum dipilih',
                    icon: 'info',
                    showConfirmButton: false
                });
            ...
            if (!$('#start').val()) {
                Swal.fire({
                    title: 'Jam mulai belum dimasukkan',
                    icon: 'info',
                    showConfirmButton: false
                });
            ...
            if (!$('#stop').val()) {
                Swal.fire({
                    title: 'Jam stop belum dimasukkan',
                    icon: 'info',
                    showConfirmButton: false
                });
            ...
            Swal.fire({
                title: 'Konfirmasi Pemakaian BBM',
                text: "Pastikan data pemakaian telah sesuai",
                icon: 'warning',

...
@section('title', 'Tambah Inventaris Kantor')
...
<main class="p-4">
    ...
    <h2 class="anchor mb-5 mt-5">Inventaris Kantor</h2>
    ...
    <section class="card shadow-sm rounded-2xl">
        ...
        <form method="POST" class="px-4">
            ...
            <div class="form-group mb-10">
                ...
                <input type="number" min="1" id="qty" name="qty" class="form-control border"
    ...
    $('#add-btn').click(function () {
        ...
        $('#list-item').append(`
            <div id="item-${item.uuid}" class="item px-4 py-3 border rounded-2xl">
                <h4>${item.name} - ${item.merk ?? 'Non Merk'}</h4>
                <h5>${qty} ${item.master_unit.name}</h5>
                <span class="badge badge-danger mb-3">Berlaku hingga: ${moment(expired_at).format('D MMM YYYY')}</span> <br/>
    ...
    $('#submit-btn').click(function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                localStorage.setItem('office-inventory-date', JSON.stringify(date));
                localStorage.setItem('office-inventories', JSON.stringify(items));
                document.location.href = "{{route('stock-inventory-office.inventory.barcode')}}"
            }
        });
    });

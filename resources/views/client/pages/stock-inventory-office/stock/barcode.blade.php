...
<main class="p-4" style="overflow-x: hidden">
    ...
    <form action="{{route('stock-inventory-office.stock.store')}}" method="POST">
        @csrf
...
<script>
    let date = JSON.parse(localStorage.getItem('office-stock-date'));
    let items = JSON.parse(localStorage.getItem('office-stocks'));

    items.forEach((item, index) => {
        $('#list-item').append(`
            <h2 class="fs-4 mb-3 border-1 border-bottom py-4 px-4">${item.name}(${item.qty})</h2>
        ...
        for (let i = 1; i <= item.qty; i++) {
            ...
            $(`#list-code-${item.uuid}`).append(`
                <section class="download-areas rounded-2xl shadow-sm card" id="download-area-${index}" style="width: 1000px; height: 1100px;">

...
@section('title', 'Laporan Permintaan Pengiriman')
...
<div class="content flex-row-fluid" id="kt_content">
    ...
    <h3 class="pb-5">Laporan Permintaan Pengiriman</h3>

    <section class="card mb-5 p-5">
        <form action="{{route('request-item-report')}}" onchange="this.submit()">
            <div class="row">
                ...
                <div class="col-6 col-md-2">
                    ...
                    <select class="form-select border" id="status" name="status">
                        ...
                        <option value="terkonfirmasi" @selected($status=='terkonfirmasi' )>Terkonfirmasi</option>
                        <option value="siap dikirim" @selected($status=='siap dikirim' )>Siap Dikirim</option>
                        <option value="selesai" @selected($status=='selesai' )>Telah Dikirim</option>
    ...
    <section class="py-5 card p-4 rounded-3 shadow-sm">
        ...
        <table class="table table-row-dashed table-row-gray-300 gy-7">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center">{{$requestItem->formatted_created_at}}</td>
                    <td class="text-center">{{$requestItem->number}}</td>
                    <td class="text-center">{{$requestItem->masterUser->name}}</td>
                    <td class="text-center">
                        @switch($requestItem->status)
                        @case('baru')
                        <span
                            class="btn btn-light-primary text-primary py-1 px-1">{{ucwords($requestItem->status)}}</span>
                        @break
                        @case('terkonfirmasi')
                        <span
                            class="btn btn-light-success text-success py-1 px-1">{{ucwords($requestItem->status)}}</span>
                        @break
                        @case('siap dikirim')
                        <span
                            class="btn btn-light-warning text-warning py-1 px-1">{{ucwords($requestItem->status)}}</span>
                        @break
                        @case('selesai')
                        <span
                            class="btn btn-light-success text-success py-1 px-1">{{ucwords($requestItem->status)}}</span>
                        ...
                        @case('ditolak')
                        <span
                            class="btn btn-light-danger text-danger py-1 px-1">{{ucwords($requestItem->status)}}</span>
                        @break
                        @endswitch
                    </td>
                    <td class="text-center">{{$requestItem->description}}</td>
                    <td class="text-center">
                        <a href="{{route('request-item-report.detail', [
        ...
        {{$requestItems->links('vendor.pagination.bootstrap-5')}}
